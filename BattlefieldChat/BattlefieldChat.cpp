...
int main() {
	...
	while (IsWindow(gameWindow)) {
		...
		if (!isFullscreenWindow(gameWindow) && !lastState && state) {
			...
			wstring str = dialog.showInputDialog(ori, gameWindow, isBorderlessWindow(gameWindow) ? 2 : 1);
			...
			if (str.length() == 0) {
				...
				// Still cannot find the pointer to message length, type some space to pad it
